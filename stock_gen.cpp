...
{
    ofstream graph_file(_file_name.c_str());
    if (!graph_file.is_open())
        return false;
   
    graph_file << "%%MatrixMarket matrix coordinate real general" << endl;
    graph_file << _vertices_count << " " << _vertices_count << " " << _edges_count << endl;

    graph_file << setprecision(22);
    for (long long i = 0; i < _edges_count; i++)
    {
        graph_file << _src_ids[i]+1 << " " << _dst_ids[i]+1;
        
        if(_weighted)
            graph_file << " " << _weights[i];
        graph_file << endl;
    }
